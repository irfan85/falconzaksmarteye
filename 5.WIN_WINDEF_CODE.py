...
def check_windows_defender_status(ip, username, password):
    try:
        ...
        if "Running" in output:
            return f"Windows Defender service is running on this Windows system ({ip}).", 5
        elif "Stopped" in output:
            return f"Windows Defender service is stopped on this Windows system ({ip}).", 0
        else:
            return f"Unable to determine Windows Defender status on this Windows system ({ip}).", 0
...
def main():
    ...
    if ping_host(ip):
        ...
        print(f"\n{Fore.GREEN}Falconzak Smart Eye is checking if Windows Defender is running on machine {ip}.{Style.RESET_ALL}\n")

        print_section_header(5, "Windows Defender Status")
        try:
